...
BoatDocuments.GetBoatDocuments = (newError, Query, mysqlConnection) => {
	return (req, res, next) => {
		try {
			...
			if (isNaN(req.params.id)) next(newError('el param "id" no es un número válido.', 400));
...
BoatDocuments.PutBoatDocuments = (newError, Query, mysqlConnection, dropbox) => {
	return (req, res, next) => {
      ...
      decodifica el string de la uri. %20 significa espacio. */
			...
			if (!req.files) next(newError("No se proporcionó ningún archivo vía multipart.", 400));

			/* Valida que se esté enviando el archivo via multipart */
			if (req.files.length != constants.boats.boatDocumentsLength)
				next(
					newError(
						"No se proporcionó la cantidad correcta de documentos: " + constants.boats.boatDocumentsLength,
						400
					)
				);
			...
			req.files.forEach((file, index) => {
				...
				if (extensions.indexOf(extension) != -1) {
					...
					let path = constants.boatDocuments.folder + decodeURIComponent(req.params.name) + "/";
					let name = ++index + "-" + date + "." + extension;

					UploadPromises.push(dropbox.UploadFile(path + name, file.buffer));
				} else {
					next(newError("No se proporcionó un archivo con extensión válida.", 400));
				}
			});
...
BoatDocuments.PutBoatDocumentByType = (newError, Query, mysqlConnection, dropbox) => {
	return (req, res, next) => {
		try {
			/* Valida manualmente el tipado de id */
			if (isNaN(req.params.id)) next(newError('el param "id" no es un número válido.', 400));
      ...
      decodifica el string de la uri. %20 significa espacio. */
			...
			if (isNaN(req.params.typeid)) next(newError('el param "typeid" no es un número válido.', 400));

			/* Valida que se esté enviando el archivo via multipart */
			if (!req.file) next(newError("No se proporcionó ningún archivo vía multipart.", 400));
