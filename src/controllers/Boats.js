...
Boats.GetBoatsByClient = (mysqlConnection) => {
	return (req, res, next) => {
		try {
			...
			// TODO: Podria estar en el esquema
			if (isNaN(parseInt(req.params.id))) next(newError('el param "clientId" no es un número válido.', 500));

			/* trae todos los barcos del cliente, y junto trae todos los engines, relaciones
      eléctricas y motores de cada barco. */
			let Promises = [
				Query(mysqlConnection, "CALL SP_READ_BOATS_BY_CLIENT (?);", [ req.params.id ]),
				Query(mysqlConnection, "CALL SP_READ_ENGINES_BY_CLIENT (?);", [ req.params.id ]),
				Query(mysqlConnection, "CALL SP_READ_BOAT_ELECTRICITY_BY_CLIENT (?);", [ req.params.id ]),
				Query(mysqlConnection, "CALL SP_READ_BOAT_DOCUMENTS_BY_CLIENT (?);", [ req.params.id ])
			];
					...
					boats.map((boat) => {
						...
						documents.forEach((document) => {
							if (document.boat_id == boat.boat_id) {
								response[boat.boat_name].documents[document.boat_document_type] = document;
							}
						});
					});
...
Boats.PutBoat = (mysqlConnection) => {
	return (req, res, next) => {
		try {
			...
			if (!/^[a-z0-9]+$/i.test(req.params.name)) next(newError('el param "name" no es un string válido.', 500));
      ...
      si son más de uno. */
          ...
          arreglo de promesas que se ejecutarán en paralelo. */
					...
					if (responsable)
						Promises.push(
							Query(mysqlConnection, "CALL SP_CREATE_RESPONSABLE (?,?,?,?,?,?);", [
								boatId,
								responsable.name,
								responsable.phone,
								responsable.email,
					...
					if (electricity)
						electricity.forEach((electr) => {
							Promises.push(
								Query(mysqlConnection, "CALL SP_CREATE_BOAT_ELECTRICITY (?,?,?);", [
									boatId,
					...
					if (documents)
						documents.forEach((doc) => {
							Promises.push(
								Query(mysqlConnection, "CALL SP_CREATE_BOAT_DOCUMENT (?,?,?);", [
									boatId,
...
Boats.DeleteBoat = (mysqlConnection) => {
	return (req, res, next) => {
		try {
			...
			/* Elimina el barco */
			Query(mysqlConnection, "CALL SP_DELETE_BOAT (?, ?);", [ req.params.id, req.params.name ])
				.then((result) => {
					/* retorna un status con el id */
					res.status(200).send(
						JSON.stringify({
							status: "Barco eliminado correctamente. Id: " + result[0][0][0].boat_id
						})
					);
				})
				.catch((error) => {
